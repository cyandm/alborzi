...
<div class="text-5xl max-xl:text-3xl max-lg:text-lg text-zinc-400 border-l relative parent w-0 col-span-1 hidden">
    <?php foreach ($desktop_menu as $menu_item):
        if (empty($menu_item->child_items))
            continue;
        ?>
        <div class="absolute inset-0 flex flex-col justify-between transition-all duration-500 opacity-0 
